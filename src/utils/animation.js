...
export function createBouncer(selector, { theme, size, speed }) {
  ...
  const angle = (Math.random() * 0.8 + 0.2) * Math.PI / 2 * (Math.random() < 0.5 ? 1 : -1) + (Math.random() < 0.5 ? Math.PI : 0);
 ...
 */
export function createGlider(selector, { theme, speed, dir, size, yAmp, yFreq, yMin, yMax, tStep = 0.015 }) {
  const el = document.querySelector(selector);
  if (!el) return null;

  let x = dir === 1 ? -size : window.innerWidth + size;
  const randomY = () => Math.random() * window.innerHeight * (yMax - yMin) + window.innerHeight * yMin;
  let baseY = randomY();
  ...
  createAnimationLoop(theme, () => {
    x += speed * dir;
    t += tStep;
    if (dir === 1 && x > window.innerWidth + size) { x = -size; baseY = randomY(); }
    if (dir === -1 && x < -size) { x = window.innerWidth + size; baseY = randomY(); }
 ...
 */
export function createDrifter(selector, { theme, speed, xAmp, xFreq, size, yMin = 40, yMaxPct = 0.5, tStep = 0.008 }) {
  const el = document.querySelector(selector);
  if (!el) return null;
  ...
  createAnimationLoop(theme, () => {
    y += dy; t += tStep;
    if (y <= yMin) { y = yMin; dy = Math.abs(dy); }
...
export function createFallingItems({ src, count, size, borderRadius = '0', stagger = 80 }) {
  for (let i = 0; i < count; i++) {
    ...
    Object.assign(el.style, {
      position: 'fixed', top: `${-(size + 10)}px`,
      left: `${Math.random() * (window.innerWidth - size)}px`,
      width: `${size}px`, height: `${size}px`,
      objectFit: 'cover', borderRadius, zIndex: '400',
    ...
    setTimeout(() => {
      (function fall() {
        ...
        if (y > window.innerHeight + size) { el.remove(); return; }
 ...
 */
export function spawnAnimatedSprite({ src, width, height, opacity = 0.5, animate, onClick = null }) {
  const id = `spawned-${++_spawnId}`;
  const div = document.createElement('div');
  div.id = id;
  Object.assign(div.style, {
    position: 'fixed', top: '0', left: '0', zIndex: '1',
    pointerEvents: 'auto', cursor: 'pointer', opacity: String(opacity),
  });
